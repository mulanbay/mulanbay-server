...
public class AccountFlowSearch extends PageSearch implements BindUser, FullEndDateTime, DateStatSearch {
    ...
    @Query(fieldName = "account.id", op = Parameter.Operator.EQ)
    private Long accountId;

    @Query(fieldName = "status", op = Parameter.Operator.EQ)
    private CommonStatus status;

    @Query(fieldName = "snapshotInfo.id", op = Parameter.Operator.NULL_NOTNULL)
    private NullType snapshot;
    ...
    public NullType getSnapshot() {
        return snapshot;
    }

    public void setSnapshot(NullType snapshot) {
